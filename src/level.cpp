...
void Level::updateDifficulty(float dt)
{
	if (game.elapsedTime - lastDifficultyUpdate >= difficultyIncrement)
	{
		...
		if (enemyLevel < 3)
			enemyLevel++; //allow more enemy types to spawn

		islandSpeed += 0.3f;

		if (enemySpawnDelay > 3.0f)
			enemySpawnDelay -= 0.2f;

		if (game.elapsedTime >= bossSpawnTime && boss == nullptr)
		{ //possibly spawn a boss
...
{
	if (obj.pos.x >= levelSize || obj.pos.x <= -levelSize || obj.pos.y >= levelSize || obj.pos.y <= -levelSize || obj.pos.z >= levelSize || obj.pos.z <= -levelSize)
	{
...
glm::vec3 Level::checkCollisionAABB(const GameObject &one, const GameObject &two)
{
	...
	if (collisionX)
	{
		if (one.pos.x <= two.pos.x)
			displacement.x = two.pos.x - (one.pos.x + (one.size.x * one.scale));
		else
			displacement.x = (two.pos.x + (two.size.x * two.scale)) - one.pos.x;
	}
	...
	if (collisionY)
	{
		if (one.pos.y <= two.pos.y)
			displacement.y = two.pos.y - (one.pos.y + (one.size.y * one.scale));
		else
			displacement.y = (two.pos.y + (two.size.y * two.scale)) - one.pos.y;
	}
	...
	if (collisionZ)
	{
		if (one.pos.z <= two.pos.z)
			displacement.z = two.pos.z - (one.pos.z + (one.size.z * one.scale));
		else
			displacement.z = (two.pos.z + (two.size.z * two.scale)) - one.pos.z;
	}
...
void Level::checkBulletEnemyCollisions(Player &player)
{
	...
	if (boss != nullptr)
		checkBulletBossCollisions(player);
}
